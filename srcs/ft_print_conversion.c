...
void						ft_print_conversion3(t_struct *data, int j)
{
	...
	else if (((j >= 7 && j <= 13 && j != 10 && j != 8) && data->l == 1) || \
	...
	{
		if (j >= 4 && j <= 6)
			ft_print_int(data, (LL)va_arg(data->ap, short int));
		if (j >= 7 && j<= 13)
			ft_print_uint(data, (ULL)va_arg(data->ap, unsigned short int));
	}
	else if (data->h == 2 && data->l == 0)
	{
		if (j >= 4 && j <= 6)
			ft_print_int(data, (LL)va_arg(data->ap, signed char));
		if (j >= 7 && j<= 13)
			ft_print_uint(data, (ULL)va_arg(data->ap, unsigned char));
	}
}
